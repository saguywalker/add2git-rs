...
fn main() {
    //let repo_path = String::from("/home/sky/lab/go-with-drone");
    //let repo = Repository::open(repo_path.as_str()).expect("Couldn't open repository");
    ...
    callbacks.credentials(|_, _, _| {
        let credentials = git2::Cred::ssh_key(
            "saguywalker",
            Some(Path::new("/home/sky/.ssh/id_rsa.pub")),
            Path::new("/home/sky/.ssh/id_rsa"),
            None
    ...
    let repo = builder.clone(repo_url, Path::new(repo_clone_path)).expect("Could not clone a repo");
    ...
    {
        //let file_path = Path::new(repo_path.as_str()).join(relative_path);
        let file_path = std::env::current_dir().unwrap().join(repo_clone_path).join(relative_path);
        let mut file = File::create(file_path.clone()).expect("Couldn't create file");
        file.write_all(b"Testing with git2").unwrap();
    }
    let commit_id = add_and_commit(&repo, &relative_path, "add example.txt").expect("Couldn't add file to repo");
    println!("New commit: {}", commit_id);

    //let _ =  push(&repo, remote_url.as_str(), Some(callbacks)).expect("Couldn't push to repo");
    ...
    let mut push_ops_with_cb = push_ops.remote_callbacks(callbacks2);
    
    let mut remote = repo.find_remote("origin").unwrap();
    remote.connect(git2::Direction::Push).unwrap();
    remote.push(&["refs/heads/master:refs/heads/master"], Some(push_ops_with_cb));
}
...
fn add_and_commit(repo: &git2::Repository, path: &Path, message: &str) -> Result<git2::Oid, git2::Error>{
